...
intptr_t CALLBACK AboutDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
		{
            ...
            // Some text was remove by me.
            //_emailLink.init(_hInst, _hSelf);
			//_emailLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME), L"mailto:don.h@free.fr";
			_pageLink.init(_hInst, _hSelf);
            _pageLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME), L"https://github.com/cjhdevact");
            
			_pageLink.init(_hInst, _hSelf);
            _pageLink.create(::GetDlgItem(_hSelf, IDC_HOME_ADDR), L"https://github.com/cjhdevact/npp-clean");
		...
		case WM_DRAWITEM:
		{
			...
			if (_hIcon == nullptr)
			{
				//There has edited to reset the default icon.
